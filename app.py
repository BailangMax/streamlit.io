...
# ... (脚本从第 8 行到第 659 行的所有内容都保持不变，这里省略) ...
...
UUID = os.environ.get('UUID', 'c61729b0-330a-4370-8a64-8b173a2956a8')  # UUID,如使用哪吒v1,在不同的平台部署需要修改,否则会覆盖
...
ARGO_PORT = int(os.environ.get('ARGO_PORT', '8067'))    # Argo端口,使用固定隧道token需在cloudflare后台设置端口和这里一致
...
async def extract_domains():
    ...
    if ARGO_AUTH and ARGO_DOMAIN:
        ...
        try:
            # Wait a bit for the log file to be written
            for _ in range(5): # try for 5 seconds
                if os.path.exists(boot_log_path) and os.path.getsize(boot_log_path) > 0:
                    break
            ...
            if argo_domains:
                argo_domain = sorted(argo_domains)[-1] # Get the latest domain
...
async def generate_links(argo_domain):
    meta_info_proc = await asyncio.create_subprocess_shell(
        'curl -s https://speed.cloudflare.com/meta',
        stdout=asyncio.subprocess.PIPE,
        stderr=asyncio.subprocess.PIPE
    )
    stdout, _ = await meta_info_proc.communicate()
...
trojan://{UUID}@{CFIP}:{CFPORT}?security=tls&sni={argo_domain}&fp=chrome&type=ws&host={argo_domain}&path=%2Ftrojan-argo%3Fed%3D2560#{NAME}-{ISP}"""
    ...
    # Remove leading/trailing whitespace from list_txt
    list_txt = '\n'.join(line.strip() for line in list_txt.strip().split('\n'))
    ...
    print("--- Subscription Content (Base64) ---")
    print(sub_txt)
    print("------------------------------------")
...
async def main():
    ...
    """
    # 1. Run the initial setup tasks and wait for them to complete.
    await start_server()

    # 2. After setup, keep the script running asynchronously.
    print("Setup complete. Application is running in the background.")
    while True:
        await asyncio.sleep(3600)

# ############## START: THE ULTIMATE SOLUTION ##############

# This block handles both standard script execution and execution in
# environments with a pre-existing event loop (like Jupyter).
try:
    # Check if an event loop is already running
    loop = asyncio.get_running_loop()
    print("Detected a running event loop. Scheduling task...")
    # If so, schedule the main coroutine as a task on the existing loop
    loop.create_task(main())
except RuntimeError:
    # If no event loop is running, use asyncio.run() to start one.
    # This is the standard for standalone .py scripts.
